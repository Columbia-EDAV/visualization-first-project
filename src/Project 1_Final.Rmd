...
```{r echo=FALSE}
barplot(prop.table(table(data$waitlist)), las=1, main='Percent of class on waitlist', cex.names=.5, horiz=TRUE, xlab='Percent of class')
```
...
barplot(by_preferred_tool / sum(by_preferred_tool), las = 1, cex.names = 0.74, ylab = "Total number", main = "Percent of users by text editor", horiz=TRUE)
```
...
```
The sankey chart shows that the preference for RStudio is explained by the large number of statistic and data-science master. However few students of statistics prefer different editors compare with Data-science where 40% of the students do not use RStudio.
...
-	Generally, Data Science students know less about all these first five tools. But they are pretty good at using Github. 
-	Other masters other than data science are good at R manipulation and R graphics. At least, none of them know nothing about these two tools.
-	Among all these six tools, experts are minority. Most of them are in statistics major.
-	Most of students, whatever his/her major is, only know a little about them. It means that we are far from to be a qualified data scientist.
...
barplot(prop.table(table(exp.by.program$exp.R.manipulation, exp.by.program$program), 2), las=1, main='R Manipulation', cex.names=.6)
barplot(prop.table(table(exp.by.program$exp.R.graphics, exp.by.program$program), 2), las=1, main='R Graphics', cex.names=.6)
barplot(prop.table(table(exp.by.program$exp.R.advanced, exp.by.program$program), 2), las=1, main='R Advanced', cex.names=.6)
barplot(prop.table(table(exp.by.program$exp.R.reproducible, exp.by.program$program), 2), las=1, main='R reproducible', cex.names=.6)
barplot(prop.table(table(exp.by.program$exp.Matlab, exp.by.program$program), 2), las=1, main='Matlab', cex.names=.6)
barplot(prop.table(table(exp.by.program$exp.Github, exp.by.program$program), 2), las=1, main='Github', cex.names=.6)
```
